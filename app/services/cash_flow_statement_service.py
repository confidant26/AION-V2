...
class CashFlowStatementService:
    ...
    async def collect_cash_flow_statements(
        ...
        statements = YahooCashFlowProvider.fetch(
            asset_id=asset.id,
            symbol=asset.symbol,
        ...
        saved_statements: list[CashFlowStatement] = []

        try:
            for statement in statements:
                saved_statement = (
                    self.cash_flow_statement_repository.upsert(
                        statement
                    )
                )
    ...
    def get_cash_flow_statements(
        ...
        statements = (
            self.cash_flow_statement_repository.get_by_asset_id(
                asset_id=asset.id,
                limit=limit,
            )
        )
